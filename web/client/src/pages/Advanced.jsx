...
export default function Advanced() {
	...
	const handleSubmit = async () => {
		...
		if (!file) return;
		setLoading(true);
		const result = await csvToArray(file, ['school', 'state', 'schoolLatitude', 'schoolLongitude']);
					...
					<Stack spacing={2} padding={2}>
						<Alert severity='warning'>
							<Typography variant='overline'>
								Upload a .csv sheet with only 4 columns: school name, state name, new latitude, and new longitude.
								<br />
								Please make sure your data is clean and that the school and state names are spelled identically as when it was uploaded.
								<br />
